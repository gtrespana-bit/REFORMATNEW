...
export const metadata: Metadata = { title: 'Fontanería Profesional', description: 'Instalaciones y reparaciones de fontanería en Carabobo.' };

export default function FontaneriaPage() {
  const svc = services.find(s => s.slug === 'fontaneria')!;
  const other = services.filter(s => s.slug !== 'fontaneria').slice(0, 3);
    ...
    <>
      <section className="relative min-h-[60vh] flex items-end overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 md:py-40">
          ...
          <h1 className="text-4xl md:text-6xl font-black leading-tight mb-4">Fontanería <span className="text-brand">Profesional</span></h1>
          <p className="text-lg text-white/50 max-w-xl">Instalaciones y reparaciones con materiales de primera calidad.</p>
      ...
      <section className="py-20 md:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16">
            <div>
              <h2 className="text-3xl font-black mb-6">Fontanería <span className="text-brand">impecable</span></h2>
              <p className="text-white/60 text-lg leading-relaxed mb-8">{svc.longDescription}</p>
              <a href={`https://wa.me/${siteConfig.phoneWa}?text=Hola! Necesito un servicio de fontanería.`} target="_blank" className="btn-glow px-8 py-4 text-base font-semibold text-white rounded-xl inline-flex items-center gap-2"><MessageCircle className="w-5 h-5" />Consultar Ahora</a>
            </div>
            <div>
