...
export const metadata: Metadata = { title: 'Pisos y Revestimientos', description: 'Instalación profesional de porcelanatos, cerámicos, vinílicos y laminados en Carabobo.' };

export default function PisosPage() {
  const svc = services.find(s => s.slug === 'pisos')!;
  const other = services.filter(s => s.slug !== 'pisos').slice(0, 3);
    ...
    <>
      <section className="relative min-h-[60vh] flex items-end overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 md:py-40">
          ...
          <h1 className="text-4xl md:text-6xl font-black leading-tight mb-4">Pisos y <span className="text-brand">Revestimientos</span></h1>
          <p className="text-lg text-white/50 max-w-xl">Instalación profesional de porcelanatos, cerámicos, vinílicos y laminados con acabados perfectos.</p>
      ...
      <section className="py-20 md:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16">
            <div>
              <h2 className="text-3xl font-black mb-6">Pisos que <span className="text-brand">definen</span> tu espacio</h2>
              <p className="text-white/60 text-lg leading-relaxed mb-8">{svc.longDescription}</p>
              <p className="text-white/60 text-lg leading-relaxed mb-8">Trabajamos con marcas reconocidas como <strong className="text-white">Porcelanosa, Daltile y más</strong> para garantizar durabilidad y estética en cada metro cuadrado.</p>
              <a href={`https://wa.me/${siteConfig.phoneWa}?text=Hola! Me interesa instalación de pisos.`} target="_blank" className="btn-glow px-8 py-4 text-base font-semibold text-white rounded-xl inline-flex items-center gap-2"><MessageCircle className="w-5 h-5" />Presupuesto para Pisos</a>
            </div>
            <div>
